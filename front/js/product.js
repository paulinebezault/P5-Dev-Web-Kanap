...
fetch(`http://localhost:3000/api/products/${id}`) // fait la requete avec l'id en paramètre
    ...
    .then(data => {
        ...
        button.onclick = () => {
            ...
            //je vais chercher dans localStorage la clé "cart" je la parse et la mets dans la variable
            if (previousCart == null) { previousCart = [] };//si pas de panier, alors tableau vide

            let alreadyExist = previousCart.findIndex((el) => el.id == data._id && el.selectedColor == colors.value);
            //si il y a un élément du tableau dont l'id correspond à l'id qu'on cherche 
            //et dont la couleur correspond à la couleur qu'on cherche
            if (alreadyExist >= 0) {
            ...
            //on modifie le locastorage avec la nouvelle valeur
